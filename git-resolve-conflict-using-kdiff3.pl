...
use warnings;
use strict;
# use diagnostics; Depends on perl_pods package on cygwin (not installed by default, https://stackoverflow.com/a/36538115/23118). Not available in got-for-windows.
...
sub find_unmerged_files {
	...
	my %tmp;
	open(PIPE, "git ls-files --unmerged $toplevel_dir -z |");
	my $content = <PIPE>;
	if ($content) {
		my @lines = split(/\0/, $content);
...
sub blob2file {
	...
	system("git cat-file blob $sha1 > \"$tempfilename\"");
	return $tempfilename;
}
...
foreach my $file (@unmerged_files) {
	...
	if ($file_removed == 2) {
		$input = prompt("Keep removed or add from ${three_descr} " . colored($file, 'green' ) . " (or skip/quit)?", "RrAaSsQq", "r");
		last if $input =~ /^[Qq]/;
		if ($input =~ /^[Aa]/) {
